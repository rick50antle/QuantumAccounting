# Extracted text: Chapter.rewritten.layered.docx

Source path: /Users/ra1/QuantumAccounting/BookChapter/Chapter.rewritten.layered.docx
...
The chapter is organized in layers: the main text gives the conceptual argument, while formal notes and the appendix state assumptions, proposition-level claims, and proof sketches for readers who want to check the logic more closely.
...
The n-account extension is subject to the same conditions. Under the conditions of the model, an n-account articulated structure can generate n bits of channel capacity when the relevant accounts satisfy the articulation constraints. The capacity scales because the relational structure preserves distinguishable patterns across accounts and statements.

Proposition 3: Scaling condition. The n-bit extension is not a free consequence of having n labels. It depends on the same articulation condition account by account. The structure scales when each relevant account contributes an independently distinguishable relational pattern.
...
The preceding analysis points to a disciplined conclusion. Quantum information theory does not prove that accounting is quantum. It does not replace accounting theory. It gives accounting researchers a new vocabulary for something accounting has long known: relationships matter.
...
The n-account extension relies on the same logic. Capacity scales when the relational structure preserves independently distinguishable patterns across accounts.
