# Extracted text: Chapter.rewritten.hardened.docx

Source path: /Users/ra1/QuantumAccounting/BookChapter/Chapter.rewritten.hardened.docx
...
The chapter is organized in layers: the main text gives the conceptual argument, while formal notes and the appendix state assumptions, proposition-level claims, and proof sketches for readers who want to check the logic more closely. Its original contribution is to treat articulation as a classical code for relational information: the old accounting architecture becomes newly legible when read beside quantum information theory.
...
# Technical Appendix: Formal Layer

## A.1 State Labels And Amplitude Bookkeeping

The accounting notation used in this chapter separates basis labels from amplitudes. The label E represents computational basis state 0, and A represents computational basis state 1. For a single qubit, a general state is not simply E or A, but alpha E + beta A, with |alpha|^2 + |beta|^2 = 1. The account label identifies the basis component; the amplitudes alpha and beta determine interference and measurement probabilities.

For a multi-account system, concatenated labels such as EA or AEA identify computational-basis product states. A general two-account state is:

psi = alpha_EE EE + alpha_EA EA + alpha_AE AE + alpha_AA AA.

The accounting notation is useful because journal-entry operations can be described on the basis labels. It is incomplete unless the corresponding amplitudes are carried along.

## A.2 Product States, Tensor Products, And Bell States

The state space of a composite quantum system is the tensor product of the component state spaces. This statement concerns spaces, not necessarily individual states. Product basis states factor; general states need not.

For example, the Bell state usually written Phi+ has the form:

Phi+ = (EE + AA) / sqrt(2).

If Phi+ were separable, it could be written as (aE + bA)(cE + dA). Comparing coefficients would require ac = 1/sqrt(2), bd = 1/sqrt(2), ad = 0, and bc = 0. These conditions cannot all hold. The state is therefore nonseparable. This is the formal reason the chapter treats Bell states as models of relational information that cannot be recovered from either component alone.

## A.3 Universality Claim

The chapter uses a narrow universality claim. The Toffoli gate is universal for classical reversible computation. Aharonov (2003) and Shi (2003) establish that Toffoli together with Hadamard gives a universal gate set for quantum computation, in the sense that arbitrary quantum operations can be approximated to arbitrary accuracy by circuits built from that set.

The accounting framework does not physically implement those circuits. The claim is that the framework can represent the relevant primitive gates. With explicit amplitude bookkeeping, this gives the framework the representational vocabulary needed to describe universal quantum computation.

## A.4 Channel Capacity Conditions

The channel-capacity model uses four messages: I, X, Z, and XZ. Because there are four messages, the maximum possible information conveyed by the message choice is two bits. A single isolated account does not generally produce four distinguishable signals because some outcomes are scalar multiples or otherwise equivalent under the model's signal criterion.

Two-bit capacity arises when the manipulated account is embedded in an articulated structure that preserves enough relational information to distinguish all four messages. In accounting terms, the important conditions are:

multiple linked statement rows;

at least two accounts in the structure;

articulation identities that constrain how account states can vary across rows.

The n-account extension relies on the same logic, but it should not be confused with the one-account/two-bit theorem. A one-account channel can reach two bits when embedded in an articulated structure with at least two qualifying accounts. The n-bit extension arises when the model expands the message space over n accounts and all relevant columns satisfy the articulation identities.

Proof sketch. The model first defines an equivalence relation on signals: scalar multiples of the same account vector are not distinct signals. That equivalence is why a single account cannot distinguish all four messages. The Z operation changes the sign of an A-type vector, but the resulting vector spans the same one-dimensional subspace, so it is not a new signal. The X operation changes the account class, so it creates a second signal class, but not four.

The two-statement/two-account case changes the object being observed. The receiver no longer observes one account in isolation; the receiver observes a structured signal in which the manipulated account is read with an unchanged second account across two rows. When each column has one A-type and one E-type element, the four possible messages generate four distinct structured signals. The second account matters because it prevents aggregation across rows from identifying signals that would otherwise collapse. In accounting language, the cash-accrual articulation supplies the code that preserves the distinction.

## A.5 Odd-Cycle Game Calculation

In the three-seat odd-cycle game, there are six equally likely seat assignments: three same-seat assignments and three adjacent-seat assignments. A classical two-coloring of a three-cycle must fail on at least one adjacent edge. Therefore the best classical strategy can win at most five of six cases, giving probability 5/6.

The entangled strategy changes the measurement relationship between Alice and Bob. In the three-seat case, the optimal entangled winning probability is:

1/2 + sqrt(3)/4 = cos^2(pi/12) = approximately 0.933.

The comparison is informative because it separates three cases: no superposition and no entanglement reproduces the classical benchmark; entanglement improves the benchmark; superposition without entanglement performs worse in the model. The result supports a precise conclusion: in this coordination problem, the useful resource is the nonseparable relationship, not mere multiplicity of possible states.
